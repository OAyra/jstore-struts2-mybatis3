...
public class EditSignonAction extends BaseAction {
    ...
    public String execute() throws Exception {
    	...
    	try {
			...
			Account _account = accountService.selectAccount(session, this.signon.getUsername());
			if( null == _account) {
				logger.debug(" -- There is no account information!! and then create the account");
				accountService.insertAccount( session, this.account);
			} else { 
				logger.debug(" -- Update Account Information...");
				accountService.updateAccount( session, this.account);
			}
			session.commit();
		} catch (Exception e) {
			session.rollback();
			logger.error(" EditSignonAction.execute() Exception: " + e.getMessage());
		} finally {
			session.close();
		}
        return SUCCESS;
