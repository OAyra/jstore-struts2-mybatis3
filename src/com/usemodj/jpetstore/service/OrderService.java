...
public class OrderService {
	...
	public List<Order> selectOrderListByUserId(SqlSession sqlSession, String userId,
		...
		OrderMapper oMapper = sqlSession.getMapper( OrderMapper.class);
		orderList = oMapper.selectOrderListByUserId( userId, rowBounds.getOffset(),rowBounds.getLimit());
	...
	public List<Order> selectOrderListByUserIdWithRowBounds(
			SqlSession sqlSession, String userId, RowBounds rowBounds) throws Exception {
		List<Order> orderList = null;
		OrderMapper oMapper = sqlSession.getMapper( OrderMapper.class);
		orderList = oMapper.selectOrderListByUserIdWithRowBounds( userId, rowBounds);
		for( Order order : orderList){
			BigDecimal totalPrice = new BigDecimal(0);
			List<LineItem> lineItems = order.getLineItems();
			for(LineItem lineItem: lineItems){
