	...
	<s:iterator value="cart.cartItemList" status="status">
     ...
     <s:url id="itemUrl" namespace="/shop" action="item_view">
      ...
      <td align="right">
	 	...
	 	<tr><td colspan="7" align="right"  style="color:red;font-weight:bold">
       ...
       &nbsp;<button type="button" onclick="document.location='<s:url namespace="/shop" action="cart_save"/>?url=<s:property value="url"/>'" <s:if test="%{cart.numberOfItems < 1}"> disabled="disabled"</s:if>><s:text name="cart.save.button" /></button>&nbsp;&nbsp;
       <button class="two" type="button" onclick="document.location='<s:url namespace="/shop" action="cart_checkout"/>'" <s:if test="%{cart.numberOfItems < 1}"> disabled="disabled"</s:if>><s:text name="checkout.button"/></button>
	 	</td>
	 	<td>&nbsp;</td>
	 	</tr>
	 	</tbody>
